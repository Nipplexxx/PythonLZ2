        ...
        if self.collide_point(*touch.pos):
            with self.canvas:
                ...
                if self.shape == "line":
                    ...
                    Ellipse(pos=(touch.x - 25 * self.size_multiplier, touch.y - 25 * self.size_multiplier),
                            size=(50 * self.size_multiplier, 30 * self.size_multiplier))

                elif self.shape == "rectangle":
                    Rectangle(pos=(touch.x - 25 * self.size_multiplier, touch.y - 25 * self.size_multiplier),
                              size=(50 * self.size_multiplier, 30 * self.size_multiplier))
...
class PaintApp(App):
    def build(self):
        ...
        ellipse_btn = Button(text="Эллипс", size_hint=(0.1, 0.1), pos_hint={"x": 0, "y": 0.4})
        ellipse_btn.bind(on_release=lambda x: self.set_shape("ellipse"))
        layout.add_widget(ellipse_btn)

        rect_btn = Button(text="Прямоугольник", size_hint=(0.1, 0.1), pos_hint={"x": 0, "y": 0.3})
        rect_btn.bind(on_release=lambda x: self.set_shape("rectangle"))
        layout.add_widget(rect_btn)

        rounded_rect_btn = Button(text="Скругление", size_hint=(0.1, 0.1), pos_hint={"x": 0, "y": 0.2})
